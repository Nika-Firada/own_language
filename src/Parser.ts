...
import StatementsNode from "./AST/StatmentsNode";
import UnarOperationNode from "./AST/UnarOperationNode";
import VariableNode from "./AST/VariableNode";
import Token from "./Token";
import TokenType, { tokenTypesList } from "./TokenTypes";
...
export default class Parser {
  ...
  parseVariableOrNumber(): ExpressionNode {
    ...
    throw new Error(
      `there is error in this position: ${this.pos}, expected number or variable`
    );
  }
  parsePrint(): ExpressionNode {
    ...
    throw new Error(`Syntax error on line ${this.pos}`);
  }
  ...
  parseExpression(): ExpressionNode {
    ...
    throw new Error(`ERrorrrrr ${this.pos}`);
  }
  ...
  run(node: ExpressionNode): any {
    ...
    if (node instanceof VariableNode) {
      if (this.scope[node.variable.text]) {
        ...
        throw new Error(
          `There is not Variable with this name - ${node.variable.text} `
        );
      }
    }
    if (node instanceof StatementsNode) {
      node.codeStrings.forEach((code) => {
        this.run(code);
      });
      return;
    }
